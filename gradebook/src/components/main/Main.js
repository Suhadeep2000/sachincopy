...
const handleFilter = (status) => {
  ...
  if (status === "passed") {
     const newFilter = Posts.filter((value) => (value.rating_grade + value.exam_grade) > 5);
     setSearchData(newFilter);
...
Posts.filter((row) => {
  ...
  let secondlast=0;
Posts.filter((row) => {
  if(((0.4 * row.rating_grade + 0.6 * row.exam_grade)>=5) && ((0.4 * row.rating_grade + 0.6 * row.exam_grade)<6)){
    secondlast++;
  }
  return firstFianl;
  });
  let last=0;
  Posts.filter((row) => {
    if(((0.4 * row.rating_grade + 0.6 * row.exam_grade)>=6) && ((0.4 * row.rating_grade + 0.6 * row.exam_grade)<7)){
      last++;
    }
    return firstFianl;
    });
...
let biggest=0;
Posts.filter((row) => {
  if(((0.4 * row.rating_grade + 0.6 * row.exam_grade)>=8)){
    biggest++;
  }
  return firstFianl;
  });
      ...
      <tbody>
        {searchData.map((row, index) => (
          <tr key={index}>
            ...
            <td>{row.rating_grade + row.exam_grade > 5 ? "passed" : "failed"}</td>
